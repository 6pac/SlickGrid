"use strict";
describe('Example - Row Grouping Titles', function () {
    var fullPreTitles = ['', 'Common Factor', 'Period', 'Analysis'];
    var fullTitles = ['#', 'Title', 'Duration', 'Start', 'Finish', '% Complete', 'Effort Driven'];
    ...
    it('should have exact Column Pre-Header & Column Header Titles in the grid', function () {
        cy.get('#myGrid')
            .find('.slick-header-columns:nth(0)')
            .children()
            .each(function ($child, index) { return expect($child.text()).to.eq(fullPreTitles[index]); });
        cy.get('#myGrid')
            .find('.slick-header-columns:nth(1)')
            .children()
            .each(function ($child, index) { return expect($child.text()).to.eq(fullTitles[index]); });
    });
    it('should have a frozen grid on page load with 3 columns on the left and 4 columns on the right', function () {
    ...
    it('should click on the "Remove Frozen Columns" button to switch to a regular grid without frozen columns and expect 7 columns on the left container', function () {
    ...
    it('should click on the "Set 3 Frozen Columns" button to switch frozen columns grid and expect 3 frozen columns on the left and 4 columns on the right', function () {
    ...
    it('should click on the Grid Menu command "Clear Frozen Columns" to switch to a regular grid without frozen columns and expect 7 columns on the left container', function () {
        cy.get('#myGrid')
