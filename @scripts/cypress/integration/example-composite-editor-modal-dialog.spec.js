"use strict";
describe('Example - Composite Editor Modal with Create/Edit/Mass-Update/Mass-Selection', function () {
    var GRID_ROW_HEIGHT = 25;
    var titles = ['', 'Title', 'Description', 'Duration', '% Complete', 'Start', 'Finish', 'Effort Driven'];
    beforeEach(function () {
    ...
    it('should expect first row to include "Task 0" and other specific properties', function () {
    ...
    it('should open the Edit Modal and expect same data to include "Task 0" and other specific properties', function () {
        ...
        cy.get('[data-editorid=title] input')
            .invoke('val')
            .then(function (text) { return expect(text).to.eq('Task 0'); });
        cy.get('[data-editorid=duration] input')
            .invoke('val')
            .then(function (text) { return expect(text).to.eq('5'); });
        cy.get('[data-editorid=start] input')
            .invoke('val')
            .then(function (text) { return expect(text).to.eq('01/01/2009'); });
        cy.get('[data-editorid=finish] input')
            .invoke('val')
            .then(function (text) { return expect(text).to.eq('01/05/2009'); });
    });
    it('Should try to change "Duration" below 5 and expect it to become invalid', function () {
        cy.get('[data-editorid=duration] input')
            .type('2');
    ...
    it('Should try to change "Duration" over 5 and expect it to become valid', function () {
        cy.get('[data-editorid=duration] input')
            .type('7')
            .blur();
        cy.get('.item-details-validation.editor-duration')
    ...
    it('Should change a few more values in the Edit Modal then expect the updated data in the grid after Saving', function () {
        cy.get('[data-editorid=title] input')
            .type('Task 0000');
    ...
    it('should open the Mass Update try to change "Duration" below 5 and expect it to become invalid', function () {
    ...
    it('Should expect to see "Duration" of "27 days" and "Effort-Driven" to be enabled accross the entire grid', function () {
        cy.get("[style=\"top:" + GRID_ROW_HEIGHT * 0 + "px\"] > .slick-cell:nth(1)").should('contain', 'Task 0000');
        cy.get("[style=\"top:" + GRID_ROW_HEIGHT * 0 + "px\"] > .slick-cell:nth(3)").should('contain', '27 days');
    ...
    it('Should expect an Alert about missing row selection before executing Mass Selection', function () {
    ...
    it('Should select row 2 and 3 and change ', function () {
    ...
    it('Should open a Create Modal window and click on Effort-Driven then expect 3 validation errors', function () {
    ...
    it('Should fill in the Create Item form then expect the new data to show up in the grid after Saving', function () {
        cy.get('[data-test=create-button]').click();
        cy.get('.modal-header > h5').contains('Inserting New Task');
        cy.get('[data-editorid=title] input')
            .type('Task 8899');
    ...
    it('Should expect to see "Duration" of "27 days" and "Effort-Driven" to be enabled accross the entire grid', function () {
        ...
        cy.get("[style=\"top:12500px\"] > .slick-cell:nth(4)").each(function ($cell) {
            var htmlText = $cell.html();
