...
export class BindingEventService {
  ...
  bind(element: Element | Window | Document | Array<Element>, eventName: string, listener: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions, groupName = '') {
    if (Array.isArray(element)) {
      element.forEach((item) => this.bind(item, eventName, listener, options, groupName));
    } else if (element) {
 ...
 *
 * Keeping this controller in the core module makes it available to separately
 * built IIFE files as `Slick.DockingController` when `slick.core.js` loads.
 */
export class DockingController<C extends Column = Column> {
  ...
  resolveColumns(columns: C[], scrollLeft: number, viewportWidth: number, leadingSide: DockingSide = 'left'): ColumnDockingLayout {
    ...
    // only when it reaches the bare viewport edge.
    ...
    // Right stickies use the symmetrical occupied edge.
  ...
  resolveRows(
    ...
    for (const [index, topRow] of stickyTopByIndex) {
      ...
      if (topDistance <= bottomDistance) {
        stickyBottom.splice(stickyBottom.findIndex((row) => row.index === index), 1);
      } else {
        stickyTop.splice(stickyTop.findIndex((row) => row.index === index), 1);
      }
    }
