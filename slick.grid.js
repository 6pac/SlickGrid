  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function setupColumnResize() {
      ...
      columnElements.each(function (i, e) {
        if (i >= columns.length || columns[i].hidden) { return; }
    ...
    function applyColumnHeaderWidths() {
      if (!initialized) { return; }
      var h, vc = getVisibleColumns();
      
      for (var i = 0, headers = $headers.children(), ii = vc.length; i < ii; i++) {
        h = $(headers[i]);
        if (jQueryNewWidthBehaviour) {
            if (h.outerWidth() !== vc[i].width) {
              h.outerWidth(vc[i].width);
            }
        } else {
            if (h.width() !== vc[i].width - headerColumnWidthDiff) {
              h.width(vc[i].width - headerColumnWidthDiff);
            }
        }
      }
