describe('Example - colspan/rowspan - Employees Timesheets', { retries: 1 }, () => {
  ...
  it('should expect the first column to be pinned on the left', () => {
    cy.get('#myGrid .slick-pinned-left-cells .slick-cell.l0.slick-cell-pinned-left').should('have.length', 10);
    cy.get('#myGrid .slick-scrolling-cells .slick-cell').should('have.length.above', 60);
  });
  ...
  describe('Basic Key Navigations', () => {
    beforeEach(() => {
      cy.window().then((win) => win.eval('grid.scrollToX(0)'));
    });
