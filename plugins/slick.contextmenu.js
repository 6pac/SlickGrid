(function ($) {
  ...
  /***
   * A plugin to add Context Menu (mouse right+click)
   *
   * USAGE:
   *
  ...
  function ContextMenu(gridOptions) {
    ...
    function init(grid) {
      _grid = grid;
      _contextMenuProperties = $.extend(true, {}, _defaults, gridOptions.contextMenu);
    ...
    function populateOptionItems(contextMenu, $optionMenu) {
      ...
      for (var i = 0, ln = contextMenu.optionItems.length; i < ln; i++) {
        ...
        $('<span class="slick-context-menu-content"></span>')
          .text(item.title)
          .appendTo($li);
      }
    }
