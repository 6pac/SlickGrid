...
(() => {
  ...
  var SlickGrid = class {
    ...
    createColumnHeaders() {
      ...
      for (let i = 0; i < this.columns.length; i++) {
        ...
        if (m.sortable && (header.classList.add("slick-header-sortable"), Utils.createDomElement("div", { className: `slick-sort-indicator ${this._options.numberedMultiColumnSort && !this._options.sortColNumberInSeparateSpan ? " slick-sort-indicator-numbered" : ""}` }, header), this._options.numberedMultiColumnSort && this._options.sortColNumberInSeparateSpan && Utils.createDomElement("div", { className: "slick-sort-indicator-numbered" }, header)), this.trigger(this.onHeaderCellRendered, {
          ...
          let headerRowCell = Utils.createDomElement("div", { className: `ui-state-default slick-state-default slick-headerrow-column l${i} r${i}` }, headerRowTarget), classname2 = this.hasFrozenColumns() && i <= this._options.frozenColumn ? "frozen" : null;
      ...
      let columnScrollTimer = null, scrollColumnsRight = () => this._viewportScrollContainerX.scrollLeft = this._viewportScrollContainerX.scrollLeft + 10, scrollColumnsLeft = () => this._viewportScrollContainerX.scrollLeft = this._viewportScrollContainerX.scrollLeft - 10, canDragScroll, sortableOptions = {
        ...
        onEnd: (e) => {
          ...
          if (cancel || !this.getEditorLock().commitCurrentEdit())
            return;
          let reorderedIds = (_a = this.sortableSideLeftInstance) == null ? void 0 : _a.toArray();
          reorderedIds = reorderedIds.concat((_b = this.sortableSideRightInstance) == null ? void 0 : _b.toArray());
    ...
    measureCellPaddingAndBorder() {
      ...
      style["box-sizing"] !== "border-box" && style["-moz-box-sizing"] !== "border-box" && style["-webkit-box-sizing"] !== "border-box" && (h.forEach((val) => this.headerColumnWidthDiff += Utils.toFloat(style[val])), v.forEach((val) => this.headerColumnHeightDiff += Utils.toFloat(style[val]))), el.remove();
      let r = Utils.createDomElement("div", { className: "slick-row" }, this._canvas[0]);
      el = Utils.createDomElement("div", { className: "slick-cell", id: "", style: { visibility: "hidden" }, textContent: "-" }, r), style = getComputedStyle(el), style["box-sizing"] !== "border-box" && style["-moz-box-sizing"] !== "border-box" && style["-webkit-box-sizing"] !== "border-box" && (h.forEach((val) => this.cellWidthDiff += Utils.toFloat(style[val])), v.forEach((val) => this.cellHeightDiff += Utils.toFloat(style[val]))), r.remove(), this.absoluteColumnMinWidth = Math.max(this.headerColumnWidthDiff, this.cellWidthDiff);
    }
    createCssRules() {
      let template = Utils.createDomElement("template", { innerHTML: '<style type="text/css" rel="stylesheet" />' });
    ...
    legacyAutosizeColumns() {
      var _a, _b;
      let i, c, widths = [], shrinkLeeway = 0, total = 0, prevTotal = 0, availWidth = this.viewportHasVScroll ? this.viewportW - ((_b = (_a = this.scrollbarDimensions) == null ? void 0 : _a.width) != null ? _b : 0) : this.viewportW;
      for (i = 0; i < this.columns.length; i++)
    ...
    updateColumnProps() {
      ...
      for (let i = 0; i < this.columns.length; i++) {
        ...
        let m = this.columns[i] = Utils.extend({}, this._columnDefaults, this.columns[i]);
        m.autoSize = Utils.extend({}, this._columnAutosizeDefaults, m.autoSize), this.columnsById[m.id] = i, m.minWidth && (m.width || 0) < m.minWidth && (m.width = m.minWidth), m.maxWidth && (m.width || 0) > m.maxWidth && (m.width = m.maxWidth);
      }
    }
    /**
    ...
    setOptions(args, suppressRender, suppressColumnSet, suppressSetOverflow) {
      ...
      this._options = Utils.extend(this._options, args), this.trigger(this.onSetOptions, { optionsBefore: originalOptions, optionsAfter: this._options }), this.validateAndEnforceOptions(), this.setFrozenOptions(), args.frozenBottom !== void 0 && (this.enforceFrozenRowHeightRecalc = !0), this._viewport.forEach((view) => {
    ...
    cleanUpCells(range, row) {
      ...
      for (; (cellToRemove = cellsToRemove.pop()) != null; )
        cellNode = cacheEntry.cellNodesByColumnIdx[cellToRemove], this._options.enableAsyncPostRenderCleanup && ((_a = this.postProcessedRows[row]) != null && _a[cellToRemove]) ? this.queuePostProcessedCellForCleanup(cellNode, cellToRemove, row) : cellNode.parentElement.removeChild(cellNode), delete cacheEntry.cellColSpans[cellToRemove], delete cacheEntry.cellNodesByColumnIdx[cellToRemove], this.postProcessedRows[row] && delete this.postProcessedRows[row][cellToRemove], totalCellsRemoved++;
    }
    cleanUpAndRenderCells(range) {
      ...
      if (!stringArray.length)
        return;
      let x = Utils.createDomElement("div", { innerHTML: this.sanitizeHtmlString(stringArray.join("")) }), processedRow, node;
    ...
    renderRows(range) {
      ...
      if (!rows.length)
        return;
      let x = Utils.createDomElement("div", { innerHTML: this.sanitizeHtmlString(stringArrayL.join("")) }), xRight = Utils.createDomElement("div", { innerHTML: this.sanitizeHtmlString(stringArrayR.join("")) });
      ...
      if (this.postProcessedCleanupQueue.length > 0) {
        ...
        for (; this.postProcessedCleanupQueue.length > 0 && this.postProcessedCleanupQueue[0].groupId == groupId; ) {
          ...
          if ((entry == null ? void 0 : entry.actionType) === "R" && entry.node.forEach((node) => {
            ...
            let column = this.columns[(_a = entry.columnIdx) != null ? _a : ""];
    ...
    handleClick(evt) {
      let e = evt;
      if (e instanceof SlickEventData ? e = evt.getNativeEvent() : evt = void 0, !this.currentEditor && (e.target !== document.activeElement || e.target.classList.contains("slick-cell"))) {
      ...
      if (!(!cell || this.currentEditor !== null && this.activeRow == cell.row && this.activeCell == cell.cell) && (evt = this.trigger(this.onClick, { row: cell.row, cell: cell.cell }, evt || e), !evt.isImmediatePropagationStopped() && this.canCellBeActive(cell.row, cell.cell) && (!this.getEditorLock().isActive() || this.getEditorLock().commitCurrentEdit()))) {
        ...
        let preClickModeOn = e.target && e.target.className === preClickClassName, column = this.columns[cell.cell], suppressActiveCellChangedEvent = !!(this._options.editable && column && column.editor && this._options.suppressActiveCellChangeOnEdit);
    ...
    makeActiveCellEditable(editor, preClickModeOn, e) {
      ...
      let metadata = (_b = (_a = this.data) == null ? void 0 : _a.getItemMetadata) == null ? void 0 : _b.call(_a, this.activeRow);
      metadata = metadata == null ? void 0 : metadata.columns;
      ...
      }), item && this.currentEditor && (this.currentEditor.loadValue(item), preClickModeOn && ((_c = this.currentEditor) != null && _c.preClick) && this.currentEditor.preClick()), this.serializedEditorValue = (_d = this.currentEditor) == null ? void 0 : _d.serializeValue(), (_e = this.currentEditor) != null && _e.position && this.handleActiveCellPositionChange();
    }
